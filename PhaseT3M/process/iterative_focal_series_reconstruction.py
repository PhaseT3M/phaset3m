...
class FocalSeriesReconstruction(
    ...
    def preprocess(
        ...
        **kwargs,
        ...
        if (self._A1a_ind is None or self._A1b_ind is None) and (
            getattr(self._datastack, "A1x", None) is not None
            or getattr(self._datastack, "A1y", None) is not None
        ):
            warnings.warn(
                (
        ...
        for tilt_index in tqdmnd(
            ...
            if self._A1a_ind is not None and getattr(self._datastack, "A1x", None) is not None:
                self._aberrations_coefs[tilt_index, :, self._A1a_ind] = xp.array(self._datastack.A1x)
            if self._A1b_ind is not None and getattr(self._datastack, "A1y", None) is not None:
                self._aberrations_coefs[tilt_index, :, self._A1b_ind] = xp.array(self._datastack.A1y)
