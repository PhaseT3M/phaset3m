...
class Visualize_tools:
    ...
    def _visualize_last_iteration(
        ...
        plot_abs_aberration_function: bool,
        plot_nm_aberration_function: Tuple[int, int],
        ...
        **kwargs,
        ...
        """
        Displays last reconstructed object and aberration function iterations.
        ...
        plot_phase_aberration_function: bool
            If true, the phase aberration function is also displayed
        plot_abs_aberration_function: bool, optional
            If true, the abs aberration function is displayed
        projection_angle_deg: float
        ...
        aberration_function_shape = self.chi_function[0,0].shape
        ...
        if plot_abs_aberration_function:
        ...
        if plot_convergence:
            if plot_phase_aberration_function or plot_abs_aberration_function:
                spec = GridSpec(
                    ncols=2,
                    nrows=2,
                    height_ratios=[4, 1],
        ...
        if plot_phase_aberration_function or plot_abs_aberration_function:
            ...
            ax.set_ylabel(r"$\mathrm{x}$ [$\mathrm{\AA}$]")
            ax.set_xlabel(r"$\mathrm{y}$ [$\mathrm{\AA}$]")
            ...
            # Initail wave
            kwargs.pop("vmin", -np.pi)
            kwargs.pop("vmax", np.pi)

            ax = fig.add_subplot(spec[0, 1])
            if plot_abs_aberration_function:
                chi_function_array = np.abs(np.fft.fftshift(np.exp(1j*self.chi_function[plot_nm_aberration_function])))
                #Complex2RGB(
                #    self.probe_fourier, hue_start=hue_start, invert=invert
                #)
                ax.set_title("Abs(aber.) {}/{}".format(plot_nm_aberration_function, self.chi_function.shape[0:2]))
                ax.set_ylabel(r"$\mathrm{k_x}$ [$\mathrm{\AA}^{-1}$]")
                ax.set_xlabel(r"$\mathrm{k_y}$ [$\mathrm{\AA}^{-1}$]")
            else:
                chi_function_array = np.angle(np.fft.fftshift(np.exp(1j*self.chi_function[plot_nm_aberration_function])))
                #Complex2RGB(
                #    self.probe, hue_start=hue_start, invert=invert
                #)
                ax.set_title("Aberration {}/{}".format(plot_nm_aberration_function, self.chi_function.shape[0:2]))
                ax.set_ylabel(r"$\mathrm{k_x}$ [$\mathrm{\AA}^{-1}$]")
                ax.set_xlabel(r"$\mathrm{k_y}$ [$\mathrm{\AA}^{-1}$]")

            im = ax.imshow(
                chi_function_array,
                extent=aberration_function_extent,
                cmap = 'PRGn',
                **kwargs,
            )
        ...
        if plot_convergence and hasattr(self, "error_iterations"):
            ...
            if plot_phase_aberration_function:
    ...
    def _visualize_all_iterations(
        ...
        **kwargs,
        ...
        if not hasattr(self, "object_iterations"):
            raise ValueError(
                (
                    "Object and aberration function iterations were not saved during reconstruction. "
        ...
        if iterations_grid == "auto":
            ...
            elif plot_phase_aberration_function or plot_abs_aberration_function:
                iterations_grid = (2, 4) if num_iter > 4 else (2, num_iter)
            else:
                iterations_grid = (2, 4) if num_iter > 8 else (2, num_iter // 2)
        else:
            if (plot_phase_aberration_function or plot_abs_aberration_function) and iterations_grid[0] != 2:
                raise ValueError()
        ...
        grid = ImageGrid(
            fig,
            spec[0],
            nrows_ncols=(1, iterations_grid[1]) if plot_phase_aberration_function else iterations_grid,
        ...
        if plot_phase_aberration_function or plot_abs_aberration_function:
            ...
            for n, ax in enumerate(grid):
                if plot_abs_aberration_function:
                    chi_function_array = np.abs(np.fft.fftshift(np.exp(1j*self.chi_function_iterations[grid_range[n]][plot_nm_aberration_function])))
                    #Complex2RGB(
                    #   asnumpy(
                    ...
                    #    invert=invert,
                    #)
                    ax.set_title(f"Iter: {grid_range[n]} Abs(aber.) {plot_nm_aberration_function}")
                    ax.set_ylabel(r"$\mathrm{k_x}$ [$\mathrm{\AA}^{-1}$]")
                    ax.set_xlabel(r"$\mathrm{k_y}$ [$\mathrm{\AA}^{-1}$]")
                else:
                    chi_function_array = np.angle(np.fft.fftshift(np.exp(1j*self.chi_function_iterations[grid_range[n]][plot_nm_aberration_function])))
                    #Complex2RGB(
                    #    probes[grid_range[n]], hue_start=hue_start, invert=invert
                    #)
                    ax.set_title(f"Iter: {grid_range[n]} Aberration {plot_nm_aberration_function}")
                    ax.set_ylabel(r"$\mathrm{k_x}$ [$\mathrm{\AA}^{-1}$]")
                    ax.set_xlabel(r"$\mathrm{k_y}$ [$\mathrm{\AA}^{-1}$]")
