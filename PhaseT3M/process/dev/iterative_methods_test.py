...
class Reconstruction_methods():
    ...
    def _Rotation_matrix_gradient_descent_adjoint(
        self,
        rotation_matrix,
        current_object,
        step_size,
        ...
        """
        Gradient for rotation matrix correction
        Computes rotation matrix  gradient and update.
        ...
        rotation_matrix_input = xp.array(rotation_matrix)

        nz, ny, nx = current_object.shape

        # --- your original coordinate construction ---
        z = (xp.arange(nz, dtype=xp.float32) - (nz - 1) / 2).reshape(-1, 1, 1) * self._pixel_sizes[1] # (Z,1,1)
        y = (xp.arange(ny, dtype=xp.float32) - (ny - 1) / 2).reshape(1, -1, 1) * self._pixel_sizes[0] # (1,Y,1)
        x = (xp.arange(nx, dtype=xp.float32) - (nx - 1) / 2).reshape(1, 1, -1) * self._pixel_sizes[1]  # (1,1,X)

        # Broadcast to full 3D grids (all shape: (Z, Y, X))
        Z, Y, X = xp.broadcast_arrays(z, y, x)

        # Stack into coordinate array: shape (3, Z, Y, X) & Flatten to (3, N) where N = Z*Y*X
        coords_flat = xp.stack((Z, Y, X), axis=0).reshape(3, -1)   # coords[0]=Z, coords[1]=Y, coords[2]=X

        # Apply rotation: rotated_coords = R @ [z,y,x]
        swap_zxy_to_xyz = xp.array([[0, 0, 1], [1, 0, 0], [0, 1, 0]])
        # swap_zxy_to_xyz = xp.array([[0, 0, 1], [0, 1, 0], [0, 0, 1]])
        # swap_zxy_to_xyz = xp.array([[1, 0, 0], [0, 1, 0], [0, 0, 1]])
        # swap_zxy_to_xyz = xp.array([[1, 0, 0], [0, 0, 1], [0, 1, 0]])
        # swap_zxy_to_xyz = xp.array([[0, 1, 0], [1, 0, 0], [0, 0, 1]])
        # swap_zxy_to_xyz = xp.array([[0, 1, 0], [1, 0, 0], [1, 0, 0]])
        new_rot_mat = swap_zxy_to_xyz.T @ rotation_matrix_input.T @ swap_zxy_to_xyz
    
        rot_flat = xp.array(new_rot_mat.T @ coords_flat)   # shape (3, N)

        rotation_matrix_input += step_size * \
            (swap_zxy_to_xyz @ (rot_flat @ (xp.array(xp.gradient(current_object)).reshape([3,-1])*self._tmp_gradient.reshape([-1])).T) @ swap_zxy_to_xyz.T).T
        
        if xp == cp:
            rotation_matrix = xp.asnumpy(rotation_matrix_input)

        return rotation_matrix
    
    def _inplane_rotation_gradient_descent_adjoint(
        self,
        ...
        rot_angle = inplane_rotation_angle*xp.pi/180
        rotation_matrix = xp.array([[xp.cos(rot_angle), -xp.sin(rot_angle)],
                                    [xp.sin(rot_angle),  xp.cos(rot_angle)]])
        gradient_rotation_matrix = xp.array([[-xp.sin(rot_angle), -xp.cos(rot_angle)],
                                              [ xp.cos(rot_angle),  -xp.sin(rot_angle)]])
        ny, nx = self._amplitudes[self._active_tilt_index,0].shape

        # --- your original coordinate construction ---
        y = (xp.arange(ny, dtype=xp.float32) - (ny - 1) / 2).reshape(-1, 1) * self._pixel_sizes[0] # (1,Y,1)
        x = (xp.arange(nx, dtype=xp.float32) - (nx - 1) / 2).reshape(1, -1) * self._pixel_sizes[1]  # (1,1,X)

        # Broadcast to full 2D grids (all shape: (Y, X))
        Y, X = xp.broadcast_arrays(y, x)

        # Stack into coordinate array: shape (2, Y, X) & Flatten to (2, N) where N = Y*X
        coords_flat = xp.stack((Y, X), axis=0).reshape(2, -1)   # coords[0]=Z, coords[1]=Y, coords[2]=X

        # Apply rotation: rotated_coords = R @ [y,x]    
        rot_flat = xp.array(rotation_matrix.T @ coords_flat)   # shape (2, N)

        tmp_grad = xp.zeros([2,ny,nx])
        for defocus_inx in range(self._amplitudes[self._active_tilt_index].shape[0]):
            tmp_grad += xp.array(xp.gradient(self._amplitudes[self._active_tilt_index,defocus_inx])) * \
                        xp.abs(xp.abs(self.predicted_exit_waves[defocus_inx])-(self._amplitudes[self._active_tilt_index,defocus_inx]))
                
        inplane_rotation_angle += step_size * xp.sum(gradient_rotation_matrix * (rot_flat.reshape([2,-1]) @ tmp_grad.reshape([2,-1]).T))
        
        return inplane_rotation_angle
    ...
    def _gradient_descent_adjoint(
        ...
        for s in reversed(range(self._num_slices)):
            ...
            if self._object_type == "complex":
                self._tmp_gradient[s] = -1j * xp.conj(obj) * xp.conj(save_waves) * residual_waves
                current_object[s] += step_size * (
                        (self._tmp_gradient[s])
                    * incident_normalization
                )
            else:
                self._tmp_gradient[s] = xp.real(-1j * xp.conj(obj) * xp.conj(save_waves) * residual_waves)
                current_object[s] += step_size * (
                        xp.real(self._tmp_gradient[s])
                    * incident_normalization
                )
    ...
    def _adjoint(
        ...
        # if not fix_rotation_matrix:
        #     self._tilt_orientation_matrices[self._active_tilt_index] = self._Rotation_matrix_gradient_descent_adjoint(
        #         self._tilt_orientation_matrices[self._active_tilt_index],
        #         current_object,
        #         rotation_matrix_step_size,
        #     )
        if not fix_inplane_rotation:
            self._inplane_rotation_angles[self._active_tilt_index] = self._inplane_rotation_gradient_descent_adjoint(
                self._inplane_rotation_angles[self._active_tilt_index],
                inplane_rotation_step_size,
            )
